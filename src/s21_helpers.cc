...
void S21Matrix::get_cofactor(S21Matrix& temp, int skip_row, int skip_col,
                             int size) const {
...
double S21Matrix::get_determinant(const S21Matrix& matrix, int size) {
  ...
  if (size == 1) {
    ...
    for (int col_index = 0; col_index < size; ++col_index) {
      ...
      res += sign * matrix.matrix_[0][col_index] *
             get_determinant(temp, temp.rows_);
      sign *= -1;
    }
  }
  return res;
...
bool S21Matrix::is_invalid_matrix() const { return (rows_ < 1 || cols_ < 1); }

bool S21Matrix::matrix_is_not_squared() const { return rows_ != cols_; }

bool S21Matrix::matrix_is_null() const { return (matrix_ == nullptr); }
